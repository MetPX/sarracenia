...
**dd_log2clusters** -b <broker> start|stop|restart|reload|status

DESCRIPTION
===========

**dd_log2clusters** is a program that reads the file `log_routing(7) <log_routing.7.html>`_.
An instance of **dd_log2clusters** is started for each line of the config file
that defines a cluster name log targer : 'cluster_Name broker_Url exchange_Log'.

Each **dd_log2clusters** instance connects to the <broker> from the command line
...
**dd_log2clusters** publish it back to the **broker_Url** under exchange **exchange_Log**.
all other notifications are ignored.


The **dd_log2clusters** command can takes 2 arguments: a broker,
...
Once connected to an AMQP broker, **dd_log2clusters** use exchange xlog, and topic v02.log.#
