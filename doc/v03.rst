...
Started on sr_subscribe, which, in the old version, is a base class for all other components,
but in v3 is just the first component that actually downloads data. So encountering all
issues with data download, and plugins that do interesting things.
...
re-implemented to have conventional return value as the number of bytes transferred,
and if they differ, that signals an issue.
...
* many use cases of sr_subscribe. this part is hairy.

* sr_sarra should be straightforward once sr_subscribe is done.
  might be zero effort.

* sr_sender send logic will then need to be exercised.
  last really hairy code.
...
* **RELEASE BLOCKER** hairy.
  sr_watch does not batch things in prefetch lumps. It just dumps an entire tree.
