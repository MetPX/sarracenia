...
* until all components are ported, will not try to fix dynamic_flow. 
  all components are ported now. need to deal with schemed plugins
  before trying dynamic flow.
...
(also ~/.config/sarra3 and ~/.cache/sarra3 ) So one can copy configurations from old to
