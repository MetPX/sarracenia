...
* md5 sums in v2 packets seem to be wrong, they disagree with v2 implementation, and 
  result from command line.

* v03_wip flow tests are now broken for v2.  Do not know when that happenned.  Need to
  keep things comparable.  broke v2 compatibility at some point in flow_tests, need 
  to find out what happenned and return v2 verification.

* issue #377 checksum n, when the file size changes, not yet addressed.
...
* in v2, download does not check the length of a file while it is downloading.
  ...
  both v2 and v3 download the file correctly, but v3 produces this error message.
