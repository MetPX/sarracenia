...
class Airnow(FlowCB):
        ...
        def poll(self):
                ...
                for Hours in range(1,3):
                     last_hour_date_time=datetime.datetime.now() - datetime.timedelta(hours=Hours)
                     Filename='HourlyData_%s.dat' % last_hour_date_time.strftime('%Y%m%d%H')
                     ...
                     if resp.ok:
                          mtime = datetime.datetime.strptime(resp.headers['last-modified'],\
                               '%a, %d %b %Y %H:%M:%S %Z')
                          last_poll = datetime.datetime.utcnow() + datetime.timedelta(seconds = -sleep)
                          logger.info(mtime)
