        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    if self.o.AllowIPs:
                        if self.remoteHost[0] not in self.o.AllowIPs:
                            logger.debug(
                                f"Connection to IP {
                                    self.remoteHost[0]} rejected. Not a part of the Accepted IPs list.")
                            conn.close()
                            raise TimeoutError
                ...
                if pid == 0:
                    ...
                    conn.close()
                    logger.info(
                        f"Forked child from host {
                            self.remoteHost[0]} with instance number {child_inst} and pid {pid}")

                    # Check if any children processes are zombies (children are killed and
                    # waiting to be terminated by parent)
        ...
        while True:
            ...
            if status == 'OK':
                ...
                try:
                    ...
                    filename = bulletinHeader + '__' + \
                        f"{randint(self.minnum, self.maxnum)}".zfill(len(str(self.maxnum)))
                    ...
                    if not binary:
                        ...
                        new_bulletin, isProblem = self.correctContents(
                            bulletin, firstchars, lines, missing_ahl, station, charset)
                        if new_bulletin != b'':
