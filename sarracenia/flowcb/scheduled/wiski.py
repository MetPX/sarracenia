...
class Wiski(Scheduled):
    ...
    def __init__(self, options):
        ...
        self.basic_auth = "Basic " + \
            base64.b64encode(f"{self.details.url.username}:{self.details.url.password}".encode()).decode()
    ...
    def gather(self, messageCountMax):  # placeholder
        ...
        for station_id in k.get_station_list().station_id:
            ...
            timeseries = k.get_timeseries_list(
                station_id=station_id, return_fields=[
                    'ts_id', 'ts_name', 'parametertype_name'])
            ...
            for i in range(len(timeseries['ts_id'].values)):
                ...
                if self.o.wiski_reject_parameterTypeName != [] and parameter_type in self.o.wiski_reject_parameterTypeName:
                    logger.debug(
                        f"Parameter type rejected due to not being specified in reject list : {
                            self.o.wiski_reject_parameterTypeName}. Skipping.")
                elif self.o.wiski_ts_name != [] and ts_name not in self.o.wiski_ts_name:
                    logger.debug(
                        f"Timeseries name rejected due to not being specified in list : {
                            self.o.wiski_ts_name}. Skipping.")
                else:
                    ...
                    if sys.platform.startswith("win"):
                        ...
                        fname = fname[0:3] + fname[3:].replace(':', '_').replace('.',
                                                                                 '_', 1).replace('+', '_').replace('-', '_')
                    else:
                        fname = f"{directory}{os.sep}ts_{ts_name}_{station_no}__{parameter_type}.{suffix}"
                    ...
                    ts = k.get_timeseries_values(ts_id=ts_id, to=now, **
                                                 {'from': then}, return_fields=self.o.wiski_return_fields)
                    if len(ts) > 0:
