    ...
    * scheduled_minute 33,45  within scheduled hours which minutes.
    ...
    def __init__(self, options, class_logger=logger):
        ...
        if self.o.scheduled_interval <= 0 and not self.appointments:
            logger.warning(
                f"no scheduled_interval or appointments (combination of scheduled_hour and scheduled_minute) set defaulting to every {
                    self.default_wait.seconds} seconds")
    ...
    def calc_next_gather_time(self, last_gather=0):
        ...
        if self.o.scheduled_interval and self.o.scheduled_interval > 0:
            ...
            logger.debug(
                f"next gather should be in {
                    self.o.scheduled_interval}s, scheduled for {
                    self.next_gather_time}")
        ...
        # No scheduled interval --> try to use configured schedule
            ...
            if next_appointment is None:
                # done for the day...
                tomorrow = datetime.datetime.fromtimestamp(
                    time.time(), datetime.timezone.utc) + datetime.timedelta(days=1)
            ...
            logger.debug(
                f"next gather scheduled for {
                    self.next_gather_time} from appointments {
                    self.appointments_to_string()}")
        ...
        # No scheduled interval and no scheduled hour/minutes/time
            ...
            logger.debug(
                f"next gather should be in {
                    self.default_wait.seconds}s, scheduled for {
                    self.next_gather_time} (default_wait")
