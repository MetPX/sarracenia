   ...
   limitations:
    ...
    def after_work(self, worklist) -> None:
        ...
        for m in worklist.ok:
            ...
            if old_blocks and 'manifest' in old_blocks:
                logger.debug(
                    f" read {len(old_blocks['manifest'])} blocks in manifest, waiting for {len(old_blocks['waiting'])} ")
            ...
            logger.info(
                f" blocks: adding block {blkno} by seeking to: {offset} to write {byteCount} bytes in {root_file}")
            ...
            if len(old_blocks['waiting']) > 0:
                ...
                with sarracenia.blockmanifest.BlockManifest(root_file) as rfm:
                    rfm.set(old_blocks)
                m.setReport(
                    206, f"file block subset {m['blocks']['number']} received and written ok. waiting for {(len(old_blocks['waiting']))} more blocks.")
