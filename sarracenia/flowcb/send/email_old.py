"""
    sarracenia.flowcb.send.email_old.Email_old is an sr3 sender plugin. Once a file is 
    posted, the plugin matches the topic(what the filename begins with) to the
    file name and sends the appropriate emails.

    "Email_old" exists for backwards compatibility reasons. The email plugin was modified
    to use accept statements (instead of file_email_to). This exists so users can continue
    using old config files without modification.
