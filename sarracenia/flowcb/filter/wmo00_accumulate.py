...
class Wmo00_accumulate(FlowCB):
    ...
    def __init__(self, options):
        ...
        if hostname_end.isnumeric():
            ...
            logger.info(
                f"sequence number first digit set to match last digit of hostname: {
                    self.sequence_first_digit} ")
        else:
    ...
    def open_accumulated_file(self):
        ...
        self.accumulated_file = f"{
            self.o.wmo00_work_directory}/{
            self.o.wmo00_origin_CCCC}{
            self.sequence_first_digit}{
                self.sequence_second_digit:01d}{
                    self.sequence:06d}.{
                        self.o.wmo00_type_marker}"
        ...
        for m in old_incoming:
            ...
            if input_data[8:11] != b'00\x01':
                if input_data[0] != SOH:
                    ...
                    n3hdr = (input_data[0:3] == b'\r\r\n') and input_data[3:6].decode(
                        'ascii').isnumeric() and (input_data[6:9] == b'\r\r\n')

                    n5hdr = (input_data[0:3] == b'\r\r\n') and input_data[3:8].decode(
                        'ascii').isnumeric() and (input_data[8:11] == b'\r\r\n')
            ...
            logger.info(
                f"appended {
                    len(input_data)} to {
                    self.accumulated_file}, offset now: {output_length} sum: {data_sum}")
            record_no += 1
