...
class AMQP(Moth):
        ...
        try:
            ...
            if queue['declare'] and queue['name']:
                ...
                if 'messageAgeMax' in self.o and self.o['messageAgeMax']:
                    ...
                    if x > 0:
                        args['x-message-ttl'] = x
                if 'type' in queue and queue['type'] is not None:
                    x = queue['type']
                    if x in ['classic', 'quorum', 'stream']:
                        args['x-queue-type'] = x
                ...
                if self.o['dry_run']:
                    ...
                    if not passive:
                        logger.info(
                            f"queue declared {
                                queue['name']} (as: {broker_str}), (messages waiting: {msg_count})")
            ...
            logger.error(
                f'connecting to: {
                    queue["name"]}, durable: {
                    queue["durable"]}, expire: {
                    queue["expire"]}, auto_delete={
                    queue["auto_delete"]}'
            )
    ...
    def putSetup(self) -> None:
        ...
        if start < self.next_connect_time:
            if start > self.next_message:
                logger.critical(
                    f"too soon to connect to {str(self.o['broker'])}. Will try again in: {self.next_connect_time - start:.2f} seconds")
                self.next_message = start + 5
            return
