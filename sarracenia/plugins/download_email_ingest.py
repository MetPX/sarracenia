...
class Fetcher(object):
    ...
    def do_download(self, parent):
        ...
        if ok:
            ...
            logger.error(
                "download_email_ingest: pollUrl has invalid credentials: %s"
                % parent.msg.baseurl)
            return False
        ...
        if "imap" in protocol:
            if protocol == "imaps":
                try:
                    ...
                    logger.error(
                        "download_email_ingest imaplib connection error: {}".
                        format(e))
                    return False
            ...
            for index in data[0].split():
                ...
                logger.info("download_email_ingest downloaded file: %s" %
                            parent.msg.new_dir + '/' + parent.msg.new_file)
                with open(parent.msg.new_dir + '/' + parent.msg.new_file,
                          'w') as f:
                    f.write(msg)
                if deleteflg == "True":
            ...
            if protocol == "pops":
                try:
                    ...
                    logger.error(
                        "download_email_ingest pop3 connection error: {}".
                        format(e))
                    return False
            ...
            logger.error(
                "download_email_ingest pollUrl protocol must be one of 'imap/imaps' or 'pop/pops'."
            )
            return False
