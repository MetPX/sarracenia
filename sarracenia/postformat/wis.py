...
class Wis(PostFormat):
   ...
   """
        ...
        """
            ...
            """
        for h in body:
            if h not in ['contentType', 'geometry', 'properties', 'size', 'baseUrl', 'relPath', 'retrievePath', 'subtopic', 'pubTime',
                         'to_clusters', 'from_cluster', 'filename', 'sundew_extension', 'mtime', 'atime', 'mode', 'identity', 'topic']:
                GeoJSONBody['properties'][h] = body[h]

        t = body['pubTime']
        GeoJSONBody['properties']['pubtime'] = t[0:4] + '-' + \
            t[4:6] + '-' + t[6:11] + ':' + t[11:13] + ':' + t[13:] + 'Z'
