...
class sr_file():
    ...
    def connect(self):
        ...
        if hasattr(self.parent, 'kbytes_ps'):
...
def file_insert_part(parent, msg, part_file):
    ...
    try:
        ...
        if fsiz != msg.length:
            parent.logger.debug("file not complete yet %s %d %d" %
    ...
    # oops something went wrong
        ...
        msg.logger.info(
            "sr_file/file_insert_part: did not insert %s " % part_file)
...
def file_process(parent):
    ...
    if not os.path.isfile(msg.relpath):
        parent.logger.warning(
            "%s moved or removed since announced" % msg.relpath)
        return True
        ...
        if ok:
            if parent.delete:
                try:
                    ...
                    msg.logger.error("delete of link to %s failed" %
                                     (msg.relpath))
            return ok
    ...
    try:
        ...
        if parent.delete:
            if msg.partflg.startswith('i'):
                msg.logger.info(
                    "delete unimplemented for in-place part files %s" %
                    (msg.relpath))
            else:
                try:
                    os.unlink(msg.relpath)
                except:
                    msg.logger.error("delete of %s after copy failed" %
                                     (msg.relpath))
...
def file_reassemble(parent):
    ...
    if not os.path.isfile(msg.target_file):
        msg.current_block = 0
        msg.set_parts('i', msg.chunksize, msg.block_count, msg.remainder,
                      msg.current_block)
        msg.set_suffix()
    ...
    while i < msg.block_count:
        ...
        if not os.path.isfile(part_file):
            msg.logger.debug(
                "part file %s not found, stop insertion" % part_file)
        ...
        if (msg.sumstr != msg.onfly_checksum):
            # Retry once
            msg.logger.warning(
                'Insertion did not complete properly, retrying...')
        ...
        msg.logger.info("Verified ingestion : block = %d of %d" %
                        (i, msg.block_count))
        i = i + 1
