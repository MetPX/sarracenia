...
class sr_post(sr_instances):
        ...
        if self.sleep > 0 and len(self.obs_watched):
            for ow in self.obs_watched:
                try:
                    ...
                    self.logger.error(
                        "Unable to unschedule event({}): {}".format(ow, err))
    ...
    def help(self):
        ...
        print(
            "                        keyword: add|cleanup|declare|disable|edit|enable|foreground|list|remove|setup"
        )
        print(
    ...
    def post_delete(self, path, key=None, value=None):
        ...
        if self.caching:
            ...
            if not new_post:
                self.logger.debug("already posted as deleted %s %s" % (path,
                                                                       sumstr))
                return False
    ...
    def post_file(self, path, lstat, key=None, value=None):
        ...
        if self.caching:
            new_post = self.cache.check(
                str(sumstr), self.post_relpath, partstr)
        ...
        # complete message
        if self.post_topic_prefix.startswith(
                'v03') and self.inline and fsiz < self.inline_max:
    ...
    def post_file_in_parts(self, path, lstat):
        ...
        if self.randomize:
            ...
            self.logger.info(
                'Sending partitions in the following order: ' + str(blocks))
        ...
        for i in blocks:
            ...
            if self.caching:
                ...
                if new_post:
                    ...
                    self.logger.debug("already posted %s (%s)" % (path,
                                                                  partstr))
                    continue
    ...
    def wakeup(self):
        ...
        for key in self.cur_events:
            ...
            try:
                ...
                self.logger.error("could not process event({}): {}".format(
                    event, err))
                self.logger.debug("Exception details:", exc_info=True)
    ...
    def walk_priming(self, p):
        ...
        if os.path.islink(p):
            ...
            self.logger.info("sr_watch %s is a link to directory %s" % (p,
                                                                        realp))
        ...
        if os.access(d, os.R_OK | os.X_OK):
            try:
                ow = self.observer.schedule(
                    self.watch_handler, d, recursive=True)
    ...
    def post_url(self,post_exchange,url,to_clusters,\
        ...
        if not self.isMatchingPattern(urlstr, self.accept_unmatch):
            self.logger.debug(
                "post of %s Rejected by accept/reject options" % urlstr)
    ...
    def start(self):
        self.logger.info("%s %s startup" % (self.program_name,
                                            self.config_name))
        self.log_settings()
        self.run()
