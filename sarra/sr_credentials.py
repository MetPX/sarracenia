...
class sr_credentials:
    ...
    def get(self, urlstr):
        ...
        self.add(urlstr)
        self.logger.debug("sr_credentials get add %s %s" %
                          (urlstr, self.credentials[urlstr]))
    ...
    def parse(self, line):
        self.logger.debug("sr_credentials parse %s" % self.pwre.sub(
            ':<secret!>@', line, count=1))
        ...
        try:
            ...
            for optval in optlist:
                ...
                if keyword == 'ssh_keyfile':
                    ...
                    self.logger.warning("bad credential option (%s)" % keyword)
