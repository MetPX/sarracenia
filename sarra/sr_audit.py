...
class sr_audit(sr_instances):
    ...
    def audit_add_user(self, u, role):
        self.logger.info("adding user %s, reset: %s, set_passwords: %s" %
                         (u, self.reset, self.set_passwords))
    ...
    def check(self):
        ...
        for user in self.users:
            ...
            if not user in picked:
                self.logger.error("unknown role '%s' for user '%s' " % (roles,
                                                                        user))
        ...
        for u in self.sources + self.subscribes:
            ...
            for e in exchange_lst:
                if e.startswith(se):
                    if e != se:
                        self.logger.warning(
                            "ok legacy user log exchange %s" % e)
                    tmp_lst.remove(e)
    ...
    def verify_pulse(self):
        ...
        try:
            ...
            self.logger.error("could not create directory ({}) with {}".format(
                self.user_config_dir, err))
    ...
    def verify_report_routing(self):
        ...
        self.logger.info(
            "sr_audit pumps using account: %s for report routing" % feeder)
        ...
        if self.report_daemons:
            for u in self.sources:
                ...
                self.logger.info(
                    "sr_audit report routing configuration source: %s, shovel: %s"
                    % (u, cfn))
                if not (os.path.isfile(cfn) or os.path.isfile(cfn + ".off")):
                    self.logger.info("creating %s" % cfn)
                    cf = open(cfn, 'w')
                    ...
                    cf.write(
                        '#     To suppress report routing, rename this file to %s.off  \n\n'
                        % os.path.basename(cfn))
                    cf.write('broker %s\n' % feeder)
            ...
            for u in self.sources + self.subscribes:
                ...
                self.logger.info(
                    "sr_audit report routing configuration subscriber: %s, shovel: %s"
                    % (u, cfn))
                if not (os.path.isfile(cfn) or os.path.isfile(cfn + ".off")):
                    self.logger.info("creating %s" % cfn)
                    cf = open(cfn, 'w')
    ...
    def verify_users(self):
        ...
        self.logger.info(
            "sr_audit verify_users subscribes = %s" % self.subscribes)
        for u in self.subscribes:
            if u in user_lst:
                user_lst.remove(u)
                if not self.reset:
            ...
            if self.users_flag:
                ...
                if username in self.admins                   or \
                   ...
                   username in self.subscribes:
                    self.logger.debug(
                        "queue ok, recognized username %s " % username)
                    continue
    ...
    def verify_pump(self):
        ...
        if not self.admin:
            ...
            self.logger.error(
                "In default.conf use : feeder amqp://fdr_user@..")
            self.logger.error("                      admin username")
            self.logger.error(
                "In credentials.conf : admin and feeder must be defined")
            error += 1
        ...
        if self.manager:
            ...
            self.logger.error(
                "Error: In default.conf use : feeder amqp://f_usern@...")
        ...
        if len(self.users) > 0:
            ...
            self.logger.error(
                "After having declared users and roles in admin.conf")
            self.logger.error(
                "you should put the needed credentials in credentials.conf")
            self.logger.error("use: sr_audit --users foreground")
            self.logger.error(
                "it creates users, set their permissions and declare their exchanges"
            )
            self.logger.error(
                "source users are also used in sr_log2source to make products log available to them"
            )
            error += 1
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                if not self.has_vip():
                    self.logger.debug(
                        "sr_audit does not have vip=%s, is sleeping" %
                        self.vip)
                    time.sleep(5)
                    continue
                else:
                    self.logger.debug(
                        "sr_audit is active on vip=%s" % self.vip)

                sleep = self.heartbeat - (
                    nowflt() - self.last_heartbeat) + 0.01
                if sleep < 0: sleep = 1

                self.logger.info(
                    "audit is sleeping %d seconds (for next heartbeat) " %
                    sleep)
    ...
    def start(self):
        self.logger.info("%s %s start" % (self.program_name,
                                          sarra.__version__))
        self.log_settings()
        self.run()
