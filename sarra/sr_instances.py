...
class sr_instances(sr_config):
    ...
    def build_instance(self, i):
        ...
        if self.config_name:
            self.instance_str += ' ' + self.config_name + ' %.2d' % i
    ...
    def build_old_path(self, i):
        ...
        if self.config_name:
            self.old_instance_str += ' ' + self.config_name + ' %.4d' % i
        ...
        if os.path.exists(self.old_logpath):
            self.logger.debug(
                "sr_instances build_old_path running old style %d" % i)
            self.old_path = True
    ...
    def cleanup_parent(self, log_cleanup=False):
        ...
        while no <= self.nbr_instances:
            ...
            if self.pid != None:
                try:
                    ...
                    self.logger.info("%s running (pid=%d)" %
                                     (self.instance_str, self.pid))
                    stopped = False
                except:
                    pass
            no = no + 1
        ...
        for x in os.listdir(dpath):
            ...
            try:
                ...
                self.logger.debug("could not delete %s on cleanup" % fpath)

        try:
            os.rmdir(dpath)
        except:
            self.logger.debug("could not delete %s on cleanup" % dpath)
    ...
    def exec_action(self, action, old=False):
        ...
        if old:
            self.logger.warning(
                "Should invoke 3: %s [args] action config" % sys.argv[0])
        ...
        if self.program_name == 'sr_post':
            ...
            if action == 'foreground':
                ...
                return
                  ...
                  or action == 'restart') and (self.sleep <= 0):
                self.logger.info(
                    "start|restart do nothing if sleep <= 0. exiting.")
                return
        ...
        if self.config_name == None:
            if action == 'list': self.exec_action_on_all(action)
                ...
                self.logger.warning(
                    "Should invoke 4: %s [args] action config" % sys.argv[0])
            os._exit(0)
        ...
        if not self.config_found:
            if action == 'add': self.exec_action_on_config(action)
                ...
                self.logger.warning(
                    "Should invoke 5: %s [args] action config" % sys.argv[0])
            os._exit(0)
    ...
    def exec_action_on_all(self, action):
        ...
        if action == 'list':
            self.print_configdir(
                ...
                os.path.normpath(self.package_dir + os.sep + 'examples' +
                                 os.sep + self.program_dir))
            self.print_configdir("general",
                                 os.path.normpath(self.user_config_dir))
            self.print_configdir("user configurations",
                                 os.path.normpath(configdir))
            ...
            try:
                if confname[-5:] == '.conf':
                    subprocess.check_output(
                        [self.program_name, action, confname])
            except:
                pass
    ...
    def exec_action_on_config(self, action):
        self.logger.debug(
            "exec_action_on_config %s, config_dir=%s, user_config=%s" %
            (action, self.config_dir, self.user_config))
        ...
        def_fil = self.user_config_dir + os.sep + sub_dir + os.sep + PureWindowsPath(
            usr_cfg).name
        ...
        if action == 'add' and not py2old:
            if not os.path.isdir(def_dir):
                try:
                    os.makedirs(def_dir, 0o775, True)
                except:
                    pass
            ...
            if not usr_fil or not os.path.isfile(usr_fil):
                self.logger.error("could not add %s to %s" % (self.user_config,
                                                              def_dir))
        ...
        # disable
            ...
            try:
                ...
                self.logger.error("cound not disable %s" % src)
        ...
        # edit
            ...
            if not usr_fil:
                ...
                if not f:
                    self.logger.error(
                        'could not identify file to edit: %s' % usr_cfg)
                    return
        ...
        # enable
            ...
            try:
                ...
                self.logger.error("cound not enable %s " % src)
        ...
        # list
            ...
            if usr_cfg == 'plugins':
                self.print_configdir(
                    "packaged plugins",
                    os.path.normpath(self.package_dir + os.sep + 'plugins'))
                self.print_configdir(
                    "user plugins",
                    os.path.normpath(
                        self.user_config_dir + os.sep + 'plugins'))
        ...
        # remove
            ...
            try:
                ...
                self.logger.error("could not remove %s" % self.def_fil)
    ...
    def reload_instance(self):
        ...
        try:
            ...
            self.logger.warning("%s no reload ... strange state; restarting" %
                                self.instance_str)
    ...
    def log_age_instance(self):
        ...
        self.logger.warning("logfile older than %d; restarting %s" %
                            (self.sanity_log_dead, self.instance_str))
        self.restart_instance()
    ...
    def restart_instance(self):
        ...
        if self.pid != None:
            self.logger.error(
                "%s could not stop... not restarted " % self.instance_str)
            return
        self.start_instance()
    ...
    def start_instance(self):
        ...
        if self.pid != None:
            try:
                ...
                self.stop_instance()
                if self.pid != None:
                    self.logger.error("%s could not stop... not started " %
                                      self.instance_str)
                    return
        ...
        if _platform == 'win32':
            ...
            #
            self.logger.debug("sys.argv[0]=%s windows_run=%s" %
                              (sys.argv[0], self.windows_run))
            for p in os.getenv("PATH").split(";"):

                if self.windows_run == 'exe':
                    q = p + '\\' + os.path.basename(sys.argv[0]).replace(
                        '.py', '') + '.exe'
                    if os.path.exists(q):
                        cmd = [q]
                        break
            ...
            if not os.path.exists(cmd[0]):
                self.logger.warning(
                    "check windows_run setting (%s), perhaps try another (pyw, py, exe)?"
                    % self.windows_run)
    ...
    def start_parent(self):
        ...
        if self.no == -1:
            ...
            if not ok:
                self.logger.error(
                    "could not write pid for instance %s" % self.instance_str)
    ...
    def status_instance(self, sanity=False):
        ...
        if psutil.pid_exists(self.pid):
            ...
            if not sanity:
                self.logger.info("%s is %s (pid=%d)" % (self.instance_str,
                                                        status, self.pid))
            return
        else:
            self.logger.info("%s instance missing (pid=%d)" %
                             (self.instance_str, self.pid))
    ...
    def stop_instance(self):
        ...
        self.logger.info("%s stopping (pid=%d)" % (self.instance_str,
                                                   self.pid))
        ...
        while stillAlive:
            ...
            try:
                p = psutil.Process(self.pid)
                self.logger.debug("stillAlive %s (pid=%d)" %
                                  (self.instance_str, self.pid))
                stillAlive = True
            except:
                stillAlive = False
            ...
            try:
                ...
                self.logger.debug("SIGTERM %s pid = %d did not work" %
                                  (self.instance_str, self.pid))
        ...
        if stillAlive:
            ...
            try:
                ...
                self.logger.debug(
                    "SIGKILL %s pid = %d, will check if still alive" %
                    (self.instance_str, self.pid))
        else:
            # if program is running... we could not stop it
            try:
                p = psutil.Process(self.pid)
                self.logger.error("unable to stop instance = %s (pid=%d)" %
                                  (self.instance_str, self.pid))
                return
            except:
                pass
    ...
    def stop_instances(self, begin, end):
        ...
        while i <= end:
            self.build_instance(i)
            pdict[i] = [
                self.pid, self.pidfile, self.pid == None, self.instance_str
            ]
            i = i + 1
        ...
        while i <= end:
            ...
            if not stopped:
                try:
                    ...
                    self.logger.debug(
                        "SIGTERM %s pid = %d, will check if still alive" %
                        (self.instance_str, self.pid))
            i = i + 1
