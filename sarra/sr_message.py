...
class sr_message():
    ...
    def content_should_not_be_downloaded(self):
        ...
        if not os.path.isfile(fname):
            self.logger.debug(
                "sr_csnbd content_match %s, file does not currently exist." %
                (fname))
            return False
        ...
        if (self.target_file == self.new_file) and (fsiz != self.filesize):
            self.logger.debug(
                "sr_csnbd %s file size different, so cannot be the same" %
                (fname))
            return False
        ...
        if self.parent.preserve_time and 'mtime' in self.headers:
            ...
            if new_mtime <= lstat[stat.ST_MTIME]:
                self.logger.debug("sr_csnbd %s new version not newer" %
                                  (fname))
                if self.log_reject:
        ...
        if self.sumflg in ['0', 'n', 'z']:
            self.logger.debug(
                "sr_csnbd content_match %s sum 0/n/z never matches" % (fname))
            return False

        if end > fsiz:
            self.logger.debug(
                "sr_csnbd content_match file not big enough... considered different"
            )
            return False
        ...
        try:
            ...
            self.logger.debug(
                "sr_csnbd something went wrong when computing local checksum... considered different"
            )
            return False

        self.logger.debug("sr_csnbd checksum in message: %s vs. local: %s" %
                          (self.local_checksum, self.checksum))
    ...
    def compute_local_checksum(self):
        self.logger.debug(
            "sr_message compute_local_checksum new_dir=%s, new_file=%s" %
            (self.new_dir, self.new_file))
        ...
        if i != self.length:
            self.logger.warning(
                "sr_message compute_local_checksum incomplete reading %d %d" %
                (i, self.length))
    ...
    def from_amqplib(self, msg=None):
        ...
        if msg:
            ...
            if msg.body[0] == '[':
                ...
                self.pubtime, self.baseurl, self.relpath, self.headers = json.loads(
                    msg.body)
                self.notice = "%s %s %s" % (self.pubtime, self.baseurl,
                                            self.relpath)
            elif msg.body[0] == '{':
                self.logger.debug("from_amqplib v03 body: %s" % msg.body)
                self.headers = json.loads(msg.body)
                self.logger.debug(
                    "from_amqplib v03 headers: %s" % self.headers)
                ...
                if "integrity" in self.headers.keys():
                    ...
                    if type(self.headers["integrity"]) is str:
                        self.headers["integrity"] = json.loads(
                            self.headers["integrity"])
                    ...
                    if sa in ['0']:
                        ...
                        sv = sum_algo_v3tov2[self.headers["integrity"][
                            "value"]]
                    else:
                        sv = encode(
                            decode(self.headers["integrity"]["value"].encode(
                                'utf-8'), "base64"), 'hex').decode('utf-8')
                    # set event.
                    if sa == 'L':
                        self.event = 'link'
                    elif sa == 'R':
                ...
                if 'blocks' in self.headers.keys():
                    ...
                    self.set_parts(parts_map[self.headers['blocks']['method']],
                ...
                if 'pulse' in self.topic:
                    ...
                    self.pubtime, self.baseurl, self.relpath = self.notice.split(
                        ' ')[0:3]
    ...
    def report_publish(self, code, message):
        ...
        if len(self.topic.encode("utf8")) >= amqp_ss_maxlen:
            ...
            self.report_topic = self.report_topic.encode("utf8")[
                0:mxlen].decode("utf8")
        ...
        if self.report_publisher != None:
            ...
            # publish
            if ok:
                self.report_publisher.publish(self.report_exchange,
                                              self.report_topic,
                                              self.report_notice, self.headers)

        self.logger.debug("%d %s : %s %s %s" %
                          (code, message, self.report_topic,
                           self.report_notice, self.hdrstr))
    ...
    def parse_v02_post(self):
        ...
        if self.mtype == 'report' or self.mtype == 'log':  # log included for compatibility... prior to rename..
            ...
            if self.version == 'v02':
                ...
                (self.report_elapse, self.report_code, self.report_host,
                 self.report_user) = self.headers['report'].split()
    ...
    def post(self, parent):
        ...
        if not (self.post_version == 'v03'):
            ...
            self.logger.debug(
                "FIXME inlining possible headers: %s" % self.headers)
        ...
        if self.post_topic_prefix != self.topic_prefix:
            self.topic = self.topic.replace(self.topic_prefix,
                                            self.post_topic_prefix, 1)
        ...
        if self.post_exchange_split > 0:
            if 'integrity' in self.headers:
                if self.headers['integrity']['method'] in ['cod', 'random']:
                    suffix = "%02d" % (ord(self.headers['integrity']['value'][
                        -1]) % self.post_exchange_split)
                    ...
                    suffix = "%02d" % (ord(self.headers['integrity']['value'][
                        -4]) % self.post_exchange_split)
                    self.logger.debug( "post_exchange_split set, keying on %s , suffix is %s" % \
                         ( self.headers['sum']['value'][-4], suffix) )
            else:
                suffix = "%02d" % (
                    ord(self.headers['sum'][-1]) % self.post_exchange_split)
                self.logger.debug(
                    "post_exchange_split set, keying on %s , suffix is %s" %
                    (self.headers['sum'][-1], suffix))
        else:
            suffix = ""
        ...
        if self.publisher != None:
            if self.post_version == 'v03':
                ...
                if "mtime" in self.headers.keys():
                    self.headers["mtime"] = timev2tov3str(
                        self.headers["mtime"])
                if "atime" in self.headers.keys():
                    self.headers["atime"] = timev2tov3str(
                        self.headers["atime"])
                ...
                body = json.dumps({
                    k: self.headers[k]
                    for k in self.headers if k not in ['sum', 'parts']
                })
                ...
                if parent.outlet == 'json':
                    ...
                    ok = self.publisher.publish(self.exchange + suffix,
                                                self.topic, body, None,
                                                self.message_ttl)
            else:
                ...
                for h in self.headers:
                    ...
                    if len(self.headers[h].encode("utf8")) >= amqp_ss_maxlen:
                        ...
                        mxlen = amqp_ss_maxlen
                        while (self.headers[h].encode("utf8")[mxlen - 1] &
                               0xc0 == 0xc0):
                            mxlen -= 1

                        self.headers[h] = self.headers[h].encode("utf8")[
                            0:mxlen].decode("utf8")
                ...
                if parent.outlet == 'json':
                    ...
                    ok = self.publisher.publish(self.exchange + suffix,
                                                self.topic, self.notice,
                                                self.headers, self.message_ttl)

        self.set_hdrstr()

        if ok:
            self.logger.debug("Published1: %s %s" % (self.exchange,
                                                     self.topic))
            self.logger.debug("Published2: '%s %s %s' %s" %
                              (self.pubtime, self.baseurl, self.relpath,
                               self.hdrstr))
        else:
            self.printlog = self.logger.error
            self.printlog("Could not publish message :")

            self.printlog("exchange %s topic %s " % (self.exchange,
                                                     self.topic))
            self.printlog("notice   %s %s %s" % (self.pubtime, self.baseurl,
                                                 self.relpath))
    ...
    def set_new(self):
        ...
        if (self.partflg != 'i') and (self.partflg != 'p'):
            self.logger.error(
                "sr_message/set_new message received with invalid partflg=%s" %
                (self.partflg))
            return
    ...
    def set_notice_url(self, url, time=None):
        ...
        if url.scheme == 'file':
            self.notice = '%s %s %s' % (self.pubtime, 'file:',
                                        '/' + notice_path)
            self.baseurl = 'file:'
    ...
    def set_parts(self,
        ...
        self.partstr = '%s,%d,%d,%d,%d' % (partflg, chunksize, block_count,
                                           remainder, current_block)
        self.headers['parts'] = self.partstr
    ...
    def verify_part_suffix(self, filepath):
        ...
        try:
            ...
            if i != fsiz:
                self.logger.warning(
                    "sr_message verify_part_suffix incomplete reading %d %d" %
                    (i, fsiz))
            ...
            self.logger.error(
                "sr_message/verify_part_suffix: incorrect extension")
