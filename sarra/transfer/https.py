...
class Https(Protocol):
    ...
    def credentials(self):
        ...
        try:
                ...
                logger.error("sr_http/credentials: unable to get credentials for %s" % self.destination)
    ...
    def get(self, remote_file, local_file, remote_offset=0, local_offset=0, length=0):
        logger.debug( "sr_http get %s %s %d" % (remote_file,local_file,local_offset))
   ...
   # ls
    ...
    def __open__(self, path, remote_offset=0, length=0):
        ...
        try:
               ...
               logger.error('Server couldn\'t fulfill the request. Error code: %s, %s' % (e.code, e.reason))
               alarm_cancel()
               raise
