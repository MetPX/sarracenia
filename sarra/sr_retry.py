...
class sr_retry:
    ...
    def msgFromJSON(self, line):
        ...
        self.logger.debug(
            'Decoding msg from json: topic={}, headers={}, notice={}'.format(
                topic, headers, notice))
        ...
        (self.message.pubtime, self.message.baseurl,
         self.message.relpath) = notice.split()
    ...
    def msg_append_to_file(self, fp, path, message, done=False):
        ...
        try:
            ...
            self.logger.error(
                "failed to serialize message to JSON: %s" % message.body)
