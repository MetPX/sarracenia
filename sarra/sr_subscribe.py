...
class sr_subscribe(sr_instances):
    ...
    def determine_move_file(self,name,relpath):
        ...
        if self.use_pattern :
           ...
           if not self.is_matching_pattern(urlstr, self.accept_unmatch) :
    ...
    def set_new(self):
        ...
        if self.current_file_option != None :
           try   :  filename  = self.sundew_get_dest_infos(filename)
           except:  self.logger.error("problem with accept file option %s" % self.current_file_option)
           token[-1] = [filename]
