...
class sr_subscribe(sr_instances):
    ...
    def connect(self):
        ...
        if self.post_broker:
            ...
            if self.post_broker != self.broker:
                ...
                self.post_hc.choose_amqp_alternative(self.use_amqplib,
                                                     self.use_pika)
    ...
    def __do_download__(self):
        ...
        if 'content' in self.msg.headers.keys():
            ...
            try:
                self.logger.debug(
                    "data inlined with message, no need to download")
                ...
                self.msg.onfly_checksum = "{},{}".format(
                    onfly_algo.registered_as(), onfly_algo.get_value())
                ...
                if self.preserve_mode and 'mode' in self.msg.headers:
                    try:
                        mode = int(self.msg.headers['mode'], base=8)
                    except:
                        mode = 0
        ...
        try:
            if scheme in self.do_downloads:
                self.logger.debug(
                    "using registered do_download for %s" % scheme)
        ...
        try:
            ...
            self.logger.error(
                "%s/__do_download__: Could not download" % self.program_name)
            self.logger.debug('Exception details: ', exc_info=True)
            if self.reportback:
    ...
    def help(self):
        ...
        if self.program_name == 'sr_sarra':
            ...
            print(
                "broker, exchange, post_broker, [post_exchange (defaults to exchange)]\n"
            )
        ...
        if self.program_name == 'sr_winnow':
            ...
            print("\nminimal configuration includes :")
            print(
                "broker, exchange, [post_broker (defaults to broker)], post_exchange\n"
            )
        ...
        if self.program_name == 'sr_subscribe':
            ...
            print(
                "\n%s subscribe.conf start # download files and display log in stdout"
                % self.program_name)
            print(
                "\n%s -d subscribe.conf start # discard files after downloaded and display log in stout"
                % self.program_name)
            print(
                "\n%s -l /tmp subscribe.conf start # download files,write log file in directory /tmp"
                % self.program_name)
            print(
                "\n%s -n subscribe.conf start # get notice only, no file downloaded and display log in stout"
                % self.program_name)
            print(
        ...
        print(
            "\tbroker amqp{s}://<user>:<pw>@<brokerhost>[:port]/<vhost>   (MANDATORY)"
        )
        ...
        if self.program_name == 'sr_subscribe':
            ...
            print(
                "\texchange             <name>          (MANDATORY if no suffix)"
            )
            print("\tpost_exchange_suffix <name>          (default none))")
        ...
        if self.program_name in ['sr_sarra', 'sr_subscribe']:
            print(
                "\tdirectory <path>     target directory (post_base_dir/directory/\"file settings\""
            )
            print(
                "\t * accept/reject following a directory option determine what is placed under it"
            )
        ...
        print(
            "\taccept_unmatch   <boolean> if no match for all accept/reject opt, accept message? (default: no).\n"
        )
        print(
            "\tevents    <event list>  msg events processed (default:create|delete|follow|link|modify)"
        )
        ...
        if self.program_name in ['sr_sarra', 'sr_subscribe']:
            ...
            print(
                "\tflatten   <string>   filename= message relpath replacing '/' by *flatten*(default:'/')"
            )
            print(
                "\tinflight  <.string>  suffix (or prefix) on filename during downloads (default: .tmp)\n"
            )
            print(
                "\tmirror    <boolean>  same directory tree as message relpath or flat. (default: True)"
            )
            print(
                "\tnotify_only|no_download  <boolean>  Do not download files (default: False)"
            )
            print(
                "\tstrip     <count>    nb. of directories to remove from message relpath. (default: 0)"
            )
            print(
                "\tbase_dir             <base_dir>      (if file is local and msg_2localfile.py is used)"
            )
            ...
            print(
                "\tyou defines the credentials in the $HOME/.config/sarra/credentials.conf"
            )
        ...
        if self.program_name == 'sr_sarra':
            ...
            print(
                "\tpost_exchange        <name>          (MANDATORY if no post_exchange_suffix set.)"
            )
            print("\tpost_exchange_suffix <name>          (default none))")

        print("\tpost_base_dir        <name>          (default None)")
        print(
            "\tpost_base_url        <url>      post message: base_url         (MANDATORY)"
        )
        if self.program_name == 'sr_sarra':
            print(
                "\tsource_from_exchange <boolean>  post message: reset headers[source] (default False)"
            )
    ...
    def __print_json(self, m):
        ...
        if m.post_version == 'v03':
            json_line = json.dumps(
                (m.pubtime, m.baseurl, m.relpath, m.headers), sort_keys=True)
        else:
            json_line = json.dumps(
                (m.topic, m.headers, m.notice), sort_keys=True)
    ...
    def __plugin_backward_compat_repare__(self):
        ...
        if self.local_dir != self.pbc_local_dir:
            self.logger.warning(
                "plugin modified parent.local_dir but should work with parent.msg.new_dir"
            )

        if self.local_file != self.pbc_local_file:
            self.logger.warning(
                "plugin modified parent.local_file but should work with parent.msg.new_dir and parent.msg.new_file"
            )

        if self.msg.local_dir != self.pbc_local_dir:
            self.logger.warning(
                "plugin modified parent.msg.local_dir but should work with parent.msg.new_dir"
            )
        ...
        if self.remote_file != self.pbc_remote_file:
            self.logger.warning(
                "plugin modified parent.remote_file but should work with parent.msg.new_file "
            )

        if self.new_url.geturl() != self.pbc_new_url.geturl():
            self.logger.warning(
                "plugin modified parent.new_url but should work with parent.msg.new_baseurl and parent.msg.new_relpath"
            )
        ...
        if self.new_dir != self.pbc_new_dir:
            self.logger.warning(
                "plugin modified parent.new_dir but should work with parent.msg.new_dir"
            )

        if self.new_file != self.pbc_new_file:
            self.logger.warning(
                "plugin modified parent.new_file but should work with parent.msg.new_file"
            )

        if self.new_baseurl != self.pbc_new_baseurl:
            self.logger.warning(
                "plugin modified parent.new_baseurl but should work with parent.msg.new_baseurl"
            )

        if self.new_relpath != self.pbc_new_relpath:
            self.logger.warning(
                "plugin modified parent.new_relpath but should work with parent.msg.new_relpath"
            )
        ...
        if self.msg.new_dir  != self.pbc_new_dir or \
           self.msg.new_file != self.pbc_new_file   :
            ...
            if self.post_base_dir:
                relpath = relpath.replace(self.post_base_dir, '', 1)
        ...
        if self.msg.time != self.msg.pubtime:
            self.logger.warning(
                "plugin modified parent.msg.time works for now, but should replace with parent.msg.pubtime"
            )
    ...
    def __plugin_backward_compat_setup__(self):
        ...
        self.pbc_new_url = urllib.parse.urlparse(
            self.msg.new_baseurl + '/' + self.msg.new_relpath)
    ...
    def process_message(self):
        ...
        if self.msg.isRetry:
            self.logger.info("Retrying notice  %s %s%s" % tuple(
                self.msg.notice.split()[0:3]))
        else:
            self.logger.debug("Received notice  %s %s%s" % tuple(
                self.msg.notice.split()[0:3]))
        ...
        if self.source_from_exchange:
            ...
            if 'from_cluster' in self.msg.headers:
                del self.msg.headers['from_cluster']
        ...
        if not 'source' in self.msg.headers:
            ...
            self.logger.debug(
                "message missing header, set default headers['source'] = %s" %
                self.msg.headers['source'])
        ...
        if not 'from_cluster' in self.msg.headers:
            if self.cluster: self.msg.headers['from_cluster'] = self.cluster
                ...
                self.msg.headers['from_cluster'] = self.broker.netloc.split(
                    '@')[-1]
            self.logger.debug(
                "message missing header, set default headers['from_cluster'] = %s"
                % self.msg.headers['from_cluster'])
        ...
        if not 'to_clusters' in self.msg.headers:
            ...
            elif self.post_broker:
                self.msg.headers[
                    'to_clusters'] = self.post_broker.netloc.split('@')[-1]
            if 'to_clusters' in self.msg.headers:
                self.logger.debug(
                    "message missing header, set default headers['to_clusters'] = %s"
                    % self.msg.headers['to_clusters'])
            else:
        ...
        if self.caching and not self.msg.isRetry:
            ...
            if not new_msg:
                if self.reportback:
                    self.msg.report_publish(304, 'Not modified1')
    ...
    def determine_move_file(self, name, relpath):
        ...
        if self.use_pattern:
            ...
            if urllib.parse.urlparse(w_msg.urlstr).path.count(
                    ":") < 1 and 'sundew_extension' in w_msg.headers.keys():
        ...
        self.logger.info(
            "while processing file rename calling on_message for oldname %s (in case the name changes)"
            % (self.msg.urlstr))
        ok = self.__on_message__()
        ...
        found = True
        try:
            os.chdir(self.msg.new_dir)
        except:
            found = False

        if not found:
            self.logger.info(
                "directory containing old file (%s) to be renamed is not present, so download required."
                % self.msg.new_dir)
            pass
        ...
        if found and name == 'newname':
            ...
            if not self.overwrite and self.msg.partstr and self.msg.content_should_not_be_downloaded(
                ...
                if self.reportback:
                    self.msg.report_publish(304, 'Not modified2')
        ...
        if found and name == 'oldname':
            ...
            if not os.path.exists(self.msg.new_file):
                self.logger.info(
                    "file to move %s not present in destination tree (download needed.)"
                    % self.msg.new_file)
                self.msg.new_dir = None
                self.msg.new_file = None
    ...
    def doit_download(self, parent=None):
        ...
        if 'newname' in self.msg.headers:
            ...
            if os.path.exists(oldpath):
                ...
                newdir, newfile, newrelp = self.determine_move_file(
                    'newname', self.msg.headers['newname'])
                ...
                if newfile != None:
                    ...
                    if not os.path.exists(newpath):
                        ...
                        if not os.path.isdir(newdir):
                            try:
                                ...
                                self.logger.warning("making %s: %s" % (newdir,
                                                                       ex))
                                self.logger.debug(
                        ...
                        try:
                            # file link
                            if os.path.isfile(oldpath) or os.path.islink(
                                    oldpath):
                                os.link(oldpath, newpath)
                                self.logger.info("move %s to %s (hardlink)" %
                                                 (oldpath, newpath))
                            # dir rename
                            if os.path.isdir(oldpath):
                                os.rename(oldpath, newpath)
                                self.logger.info("move %s to %s (rename)" %
                                                 (oldpath, newpath))
                            if self.reportback:
                                self.msg.report_publish(201, 'moved')
                            need_download = False
                        except Exception as ex:
                            self.logger.warning("mv %s %s: %s" % (oldpath,
                                                                  newdir, ex))
                            self.logger.debug(
                                'Exception details:', exc_info=True)
                        #MG FIXME : except: return False  maybe ?
                    ...
                    # if the newpath exists log a message in debug only
                        ...
                        self.logger.warning(
                            "could not move %s to %s (file exists)" %
                            (oldpath, newpath))
                else:
        ...
        if 'oldname' in self.msg.headers:
            ...
            olddir, oldfile, oldrelp = self.determine_move_file(
                ...
                if os.path.exists(oldpath):
                    ...
                    if not os.path.isdir(self.msg.new_dir):
                        try:
                            ...
                            self.logger.warning("making %s: %s" %
                                                (self.msg.new_dir, ex))
                            self.logger.debug(
                                'Exception details:', exc_info=True)
                    ...
                    try:
                        if os.path.isfile(oldpath) or os.path.islink(oldpath):
                            ...
                            self.logger.error(
                                "did not move %s to %s (rename)" % (oldpath,
                                                                    newpath))
                        need_download = False
                    except:
                        ok = False
                    ...
                    if ok:
                        self.logger.info("file moved %s to %s" % (oldpath,
                                                                  newpath))
                        need_download = False
                        if self.reportback:
                            self.msg.report_publish(201, 'moved')

                        self.msg.set_topic(self.post_topic_prefix,
                                           self.msg.new_relpath)
                        self.msg.set_notice(self.msg.new_baseurl,
                                            self.msg.new_relpath,
                                            self.msg.pubtime)
                        ...
                        if self.post_broker:
                            ...
                            if self.outlet == 'json':
                                ...
                                print("%s" % '/'.join(
                                    self.msg.notice.split()[1:3]))
                        if self.msg.isRetry: self.consumer.msg_worked()
                        return True

                    self.logger.debug("could not move %s to %s (hardlink)" %
                                      (oldpath, newpath))
                    self.logger.info("newfile %s will be downloaded" % newpath)
            else:
                self.logger.debug(
                    "oldfile is None, so newpath=%s will be downloaded." %
                    (newpath))
        #=================================
        ...
        if self.msg.event == 'delete':
            ...
            if not 'delete' in self.events and not 'newname' in self.msg.headers:
                self.logger.info(
                    "message to remove %s ignored (events setting)" %
                    self.msg.new_file)
                if self.msg.isRetry: self.consumer.msg_worked()
                return True
            ...
            try:
                ...
                self.logger.error(
                    "sr_subscribe/doit_download: could not remove %s." % path)
                self.logger.debug('Exception details: ', exc_info=True)
                if self.reportback:
            ...
            self.msg.set_notice(self.msg.new_baseurl, self.msg.new_relpath,
                                self.msg.pubtime)
            if 'newname' in self.msg.headers:
                self.msg.headers['newname'] = newname
            if self.post_broker:
                ok = self.__on_post__()
                if ok and self.reportback:
                    self.msg.report_publish(201, 'Published')
        ...
        if self.msg.event == 'link':
            self.logger.debug("message is to link %s to %s" %
            ...
            if not os.path.isdir(self.msg.new_dir):
                try:
                    ...
                    self.logger.warning("making %s: %s" % (self.msg.new_dir,
                                                           ex))
                    self.logger.debug('Exception details:', exc_info=True)
            ...
            try:
                ...
                ok = False
                self.logger.error("symlink of %s %s failed." %
                                  (self.msg.new_file,
                                   self.msg.headers['link']))
                self.logger.debug('Exception details:', exc_info=True)
                if self.reportback:
                    self.msg.report_publish(500, 'symlink failed')
        ...
        if self.post_base_dir and not '{' in self.post_base_dir:
            if not os.path.isdir(self.post_base_dir):
                self.logger.error(
                    "directory %s does not exist" % self.post_base_dir)
        ...
        try:
            ...
            self.logger.error(
                "could not cd to directory %s" % self.msg.new_dir)
            self.logger.debug('Exception details:', exc_info=True)
        ...
        if need_download:
            ...
            if self.msg.onfly_checksum:
                ...
                if (self.msg.sumflg[0] not in [
                        'a', 'z'
                ]) and (self.msg.onfly_checksum != self.msg.sumstr):
                    ...
                    self.logger.warning(
                        "onfly_checksum %s differ from message %s" %
                ...
                if self.on_data_list:
                    ...
                    self.logger.debug("setting checksum from on_data: %s" %
                                      self.msg.data_checksum)
                    new_checksum = self.msg.data_checksum

                if new_checksum:
                    # set the value so that if it gets posted later, it is correct.
                    self.msg.set_sum(
                        self.msg.sumflg.split(',')[-1], new_checksum)
            ...
            if  (not ( self.msg.event in [ 'link',  'delete' ] )  ) and \
                ...
                if fs.st_size < self.inline_max:
                    ...
                    try:
                        ...
                        self.logger.debug(
                            "inlined data for %s" % self.msg.new_file)

                    except:
                        self.logger.error(
                            "failled trying to inline %s" % self.msg.new_file)
                        self.logger.debug('Exception details:', exc_info=True)
    ...
    def restore_messages(self):
        ...
        self.logger.info("%s restoring %d messages from save %s " %
                         (self.program_name, total, self.save_path))
        ...
        with open(self.save_path, "r") as fp:
            for json_line in fp:
                ...
                if len(jt) == 3:  # v02 format...
                    ...
                    self.msg.notice = "%s %s %s" % (self.msg.pubtime,
                                                    self.msg.baseurl,
                                                    self.msg.relpath)

                elif len(jt) == 4:  # early v03 format.
                    (self.msg.pubtime, self.msg.baseurl, self.msg.relpath,
                     self.msg.headers) = jt
                    self.msg.topic = self.msg.post_topic_prefix + '.'.join(
                        self.msg.relpath.split('/')[0:-1])
                    self.msg.notice = "%s %s %s" % (self.msg.pubtime,
                                                    self.msg.baseurl,
                                                    self.msg.relpath)
                else:
                    self.logger.warning(
                        "skipped corrupt line in save file %s, line %d: %s" %
                        (self.save_path, count, json_line))
                    continue

                self.msg.from_amqplib()
                self.msg.isRetry = False
                self.logger.info("%s restoring message %d of %d: topic: %s" %
                                 (self.program_name, count, total,
                                  self.msg.topic))
                ok = self.process_message()

        if count >= total:
            self.logger.info("%s restore complete deleting save file: %s " %
                             (self.program_name, self.save_path))
            os.unlink(self.save_path)
        else:
            self.logger.error(
                "%s only restored %d of %d messages from save file: %s " %
    ...
    def run(self):
        ...
        if self.config_name and self.config_name[0:
                                                 3] == 'rr_' and not self.report_daemons:
            self.logger.info("report_daemons is False, skipping %s config" %
                             self.config_name)
            self.cleanup()
            os._exit(0)
        ...
        while True:
            try:
                ...
                if self.vip:
                    ...
                    if last != has_vip:
                        last = has_vip
                        self.logger.debug("%s is active on vip=%s" %
                                          (self.program_name, self.vip))
                ...
                self.logger.error(
                    err_msg.format(self.program_name, err, going_badly))
                self.logger.debug('Exception details: ', exc_info=True)
                if self.retry_mode:
    ...
    def save_message(self):
        ...
        if not self.msg.topic.split('.')[1] in ['post', 'report']:
            self.logger.warning(
                "%s skipping unsupported message format for saving. topic: %s"
                % (self.program_name, self.msg.topic))
            return

        self.logger.info("%s saving %d message topic: %s" %
                         (self.program_name, self.save_count, self.msg.topic))
        self.save_count += 1
        self.save_fp.write(
            json.dumps(
                (self.msg.pubtime, self.msg.baseurl, self.msg.relpath,
    ...
    def set_new(self):
        ...
        if 'rename' in self.msg.headers:
            relpath = '%s' % self.msg.headers['rename']
        ...
        # strip using a pattern
            ...
            self.logger.debug(
                'set_new, pattern stripping active: %s' % self.pstrip)

            #MG FIXME Peter's wish to have replacement in pstrip (ex.:${SOURCE}...)
            try:
                relstrip = re.sub(self.pstrip, '', relpath, 1)
            except:
                relstrip = relpath
        ...
        if self.currentFileOption != None:
            try:
                ...
                self.logger.error("problem with accept file option %s" %
                                  self.currentFileOption)
            token[-1] = [filename]
        ...
        if self.destfn_script:
            ...
            if filename != self.new_file:
                self.logger.debug("destfn_script : %s becomes %s " %
        ...
        if 'sundew_extension' in self.msg.headers.keys():
            tfname = filename.split(
                ':')[0] + ':' + self.msg.headers['sundew_extension']

        # when sr_sender did not derived from sr_subscribe it was always called
        new_dir = self.sundew_dirPattern(self.msg.urlstr, tfname, new_dir,
                                         filename)
    ...
    def cleanup(self):
        self.logger.info("%s %s cleanup" % (self.program_name,
                                            self.config_name))
    ...
    def declare(self):
        self.logger.info("%s %s declare" % (self.program_name,
                                            self.config_name))
