...
class sr_ftp(sr_proto):
    ...
    def connect(self):
        ...
        try:
            ...
            if not self.tls:
                ...
                ftp = ftplib.FTP_TLS(
                    self.host, self.user, self.password, timeout=expire)
            ...
            self.logger.error("Unable to connect to %s (user:%s)" % self.host,
                              self.user)
            self.logger.debug('Exception details: ', exc_info=True)
    ...
    def get(self,
            ...
            local_offset=0,
            length=0):
        self.logger.debug("sr_ftp get %s %s %d" % (remote_file, local_file,
                                                   local_offset))
    ...
    def mkdir(self, remote_dir):
        ...
        alarm_cancel()
        alarm_set(self.iotime)
        self.ftp.voidcmd('SITE CHMOD ' + "{0:o}".format(self.parent.chmod_dir)
                         + ' ' + remote_dir)
        alarm_cancel()
    ...
    def put(self,
        ...
        if self.binary:
            self.ftp.storbinary("STOR " + remote_file, src, self.bufsize,
                                self.write_chunk)
        else:
