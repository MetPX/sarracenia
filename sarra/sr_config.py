...
class sr_config:
    def __init__(self, config=None, args=None, action=None):
        ...
        self.programs = [
            'post', 'watch', 'winnow', 'sarra', 'shovel', 'subscribe',
            'sender', 'poll', 'report'
        ]
        ...
        self.user_old_log_dir = self.user_log_dir.replace(
            os.sep + 'log', os.sep + 'var' + os.sep + 'log')
        ...
        if os.path.isdir(var) and not os.path.islink(var):
            print(
                "sr_config __init__ migrating logs to new location (without 'var')"
            )
            if os.path.isdir(self.user_log_dir):
                print(
                    "sr_config __init__ moving old sr_log state directory out of the way to .old"
                )
                os.rename(self.user_log_dir,
                          self.user_log_dir.replace("/log", "/log.old"))
            print(
                "sr_config __init__ moving old log file directory the new location %s "
                % self.user_log_dir)
            shutil.move(self.user_old_log_dir, self.user_log_dir)
            print(
                "sr_config __init__ moving old var directory out of the way to %s.old"
                % (var))
            os.rename(var, var + ".old")
            if sys.platform != 'win32':
                print(
                    "sr_config __init__ create var symlink so no scripts need to be change for now. "
                )
                os.symlink(".", var)
        ...
        self.program_name = re.sub(r'(-script\.(pyw|py)|\.exe|\.py)?$', '',
    ...
    def log_settings(self):
        ...
        if self.program_name == 'sr_poll':
            self.logger.info(
                '\t\ton_line: %s' % ''.join(map(self.xcl, self.on_line_list)))
            self.logger.info('\t\ton_html_page: %s' % ''.join(
                map(self.xcl, self.on_html_page_list)))
            self.logger.info(
                '\t\tdo_poll: %s' % ''.join(map(self.xcl, self.do_poll_list)))

        elif self.program_name == 'sr_sender':
            self.logger.info(
                '\t\tdo_send: %s' % ''.join(map(self.xcl, self.do_send_list)))
            self.logger.info(
                '\t\tdo_put : %s' % ''.join(map(self.xcl, self.do_put_list)))

        elif self.program_name in ['sr_watch', 'sr_post']:
            self.logger.info('\t\ton_watch: %s' % ''.join(
                map(self.xcl, self.on_watch_list)))

        else:
            self.logger.info('\t\tdo_download: %s' % ''.join(
                map(self.xcl, self.do_download_list)))
            self.logger.info('\t\tdo_get     : %s' % ''.join(
                map(self.xcl, self.do_get_list)))

        self.logger.info('\t\ton_message: %s' % ''.join(
            map(self.xcl, self.on_message_list)))
        self.logger.info(
            '\t\ton_data: %s' % ''.join(map(self.xcl, self.on_data_list)))
        self.logger.info(
            '\t\ton_part: %s' % ''.join(map(self.xcl, self.on_part_list)))
        self.logger.info(
            '\t\ton_file: %s' % ''.join(map(self.xcl, self.on_file_list)))
        self.logger.info(
            '\t\ton_post: %s' % ''.join(map(self.xcl, self.on_post_list)))
        self.logger.info('\t\ton_heartbeat: %s' % ''.join(
            map(self.xcl, self.on_heartbeat_list)))
        self.logger.info(
            '\t\ton_report: %s' % ''.join(map(self.xcl, self.on_report_list)))
        self.logger.info(
            '\t\ton_start: %s' % ''.join(map(self.xcl, self.on_start_list)))
        self.logger.info(
            '\t\ton_stop: %s' % ''.join(map(self.xcl, self.on_stop_list)))
    ...
    def check_extended(self):
        ...
        for option in self.extended_options:
            ...
            self.logger.warning(
                "extended option %s = %s (unknown or not declared)" % (option,
                                                                       value))
            ok = False
    ...
    def check_for_remote_config(self, path):
        ...
        try:
            ...
            for line in f.readlines():
                words = line.split()
                if (len(words) >= 1
                        and not re.compile('^[ \t]*#').search(line)):
    ...
    def config(self, path):
        self.logger.debug(
            "sr_config config component is: %s" % self.program_name)
    ...
    def config_path(self, subdir, config, mandatory=True, ctype='conf'):
        ...
        if config.startswith('http:'):
            ...
            self.logger.debug("http url %s path %s name %s" % (urlstr, path,
                                                               name))
        ...
        config_path = os.path.join(self.user_config_dir, subdir,
                                   config_name + ext)
        ...
        config_path = os.path.join(self.site_config_dir, subdir,
                                   config_name + ext)
        self.logger.debug("config_path %s " % config_path)
    ...
    def configure_statehost(self):
        ...
        if hostdir and not hostdir in self.user_cache_dir:
            ...
            self.user_cache_dir += os.sep + self.program_name.replace(
                'sr_', '')
            self.user_cache_dir += os.sep + "%s" % self.config_name
        ...
        if not self.program_name in ['sr', 'sr_config']:
            self.logger.debug(
                "sr_config user_cache_dir  %s " % self.user_cache_dir)
            try:
                os.makedirs(self.user_cache_dir, 0o775, True)
            except Exception as ex:
                self.logger.warning("making %s: %s" % (self.user_cache_dir,
                                                       ex))
    ...
    def duration_from_str(self, str_value, setting_units='s'):
        self.logger.debug("sr_config duration_from_str %s unit %s" %
                          (str_value, setting_units))
    ...
    def execfile(self, opname, path):
        ...
        ok, script = self.config_path(
            'plugins', path, mandatory=True, ctype='py')
        if ok:
            self.logger.debug("installing %s plugin %s" % (opname, script))
        else:
            self.logger.error("installing %s plugin %s failed: not found " %
                              (opname, path))
            return False
        ...
        if getattr(self, opname) is None:
            self.logger.error("%s plugin %s incorrect: does not set self.%s" %
                              (opname, path, opname))
            return False
    ...
    def find_conf_file(self, name):
        ...
        f = self.find_file_in_dir(
            self.user_config_dir + os.sep + 'plugins', name, recursive=True)
        if f: return f
        ...
        f = self.find_file_in_dir(
            self.package_dir + os.sep + 'plugins', name, recursive=True)
        if f: return f
        ...
        for p in self.programs:
            f = self.find_file_in_dir(
                self.package_dir + os.sep + 'examples' + os.sep + p, name)
            if f: return f
    ...
    def heartbeat_check(self):
        ...
        if elapse > self.heartbeat:
            ...
            if ratio > 0.1:
                self.logger.warning(
                    "on_heartbeat spent more than 10%% of heartbeat (%d)" %
                    self.heartbeat)
    ...
    def __on_heartbeat__(self):
        ...
        for plugin in self.on_heartbeat_list:
            try:
                ...
                self.logger.error(
                    "ssr_config/__on_heartbeat__  3: plugin %s, execution failed."
                    % plugin)
                self.logger.debug('Exception details: ', exc_info=True)
        ...
        for mask in self.masks:
            ...
            if mask_regexp.match(chaine):
                if not accepting:
                    if self.log_reject:
                        self.logger.info(
                            "reject: mask=%s strip=%s pattern=%s" %
                            (str(mask), strip, chaine))
                    return False
                self.logger.debug("isMatchingPattern: mask=%s strip=%s" %
                                  (str(mask), strip))
                return True
        ...
        for d in sumdirs:
            ...
            for s in os.listdir(d):
                ...
                if not isinstance(self.add_sumalgo, sr_checksum):
                    self.logger.error(
                        "sum file %s add_sumalgo is not inherited from class sr_checksum"
                        % p)
                    continue
                ...
                if register_name == None:
                    self.logger.error(
                        "sum file %s add_sumalgo does not provide a checksum letter/name"
                        % p)
                    continue
                ...
                if register_name in self.sumalgos:
                    self.logger.error(
                        "sum file %s add_sumalgo with a checksum letter/name already set, skipped"
                        % p)
                    continue

                # add the sumalgo
                self.logger.debug(
                    "sum file %s add_sumalgo with a checksum letter/name %s" %
                    (p, register_name))
        ...
        try:
            if sys.version_info.major < 3 or (sys.version_info.major == 3
                ...
                if sc_path and cmd_list[0].startswith("sr_cp"):
                    subprocess.run(
                        [sc_path + '/' + cmd_list[0]] + cmd_list[1:],
                        check=True)
                elif sr_path and cmd_list[0].startswith("sr"):
                    subprocess.run(
                        [sr_path + '/' + cmd_list[0] + '.py'] + cmd_list[1:],
                        check=True)
                else:
    ...
    def register_plugins(self):
        ...
        for do_download in self.do_download_list:
            ...
            if hasattr(parent_class, 'registered_as'):
                ...
                if not isinstance(register_name, list):
    ...
    def sundew_getDestInfos(self, filename):
        ...
        if 'sundew_extension' in self.msg.headers.keys():
            parts = [parts[0]
                     ] + self.msg.headers['sundew_extension'].split(':')
            filename = ':'.join(parts)
        ...
        for spec in self.currentFileOption.split(':'):
            if spec == 'WHATFN':
                ...
                self.logger.error(
                    "Don't understand this DESTFN parameter: %s" % spec)
                return (None, None)
        ...
        if '$' in result:
            ...
            for e in elst:
                try:
                    ...
                    if '[' in repval and ']' in repval:
                        repval = eval('"%s" % self.' + e + '[0]')
    ...
    def option(self, words):
        ...
        try:
            if words0 in ['accept', 'get', 'reject']:  # See: sr_config.7
                ...
                self.masks.append(
                    (pattern, self.currentDir, self.currentFileOption,
                     mask_regexp, accepting, self.mirror, self.strip,
            ...
            elif words0 in ['accept_unmatched', 'accept_unmatch',
                            'au']:  # See: sr_config.7
                ...
                n = 2
                    ...
                    'base_dir', 'bd'
            ]:  # See: sr_config.7  for sr_post.1,sarra,sender,watch
                ...
                n = 2
                    ...
                    'broker', 'b'
            ]:  # See: sr_consumer.7 ++   fixme: everywhere, perhaps reduce
                urlstr = words1
                ok, url = self.validate_urlstr(urlstr)
                self.broker = url
                if not ok or not url.scheme in ['amqp', 'amqps']:
                ...
                n = 2
                    ...
                    'caching', 'cache', 'no_duplicates', 'noduplicates', 'nd',
                    'suppress_duplicates', 'sd'
            ]:  # See: sr_post.1 sr_watch.1
                if (words1 is None) or words[0][0:1] == '-':
                    self.caching = 300
                    n = 1
                else:
                    ...
                    n = 2
                if self.caching and not hasattr(self,
                                                'heartbeat_cache_installed'):
                ...
                #   self.cache = sr_cache(self) ####@
                    ...
                    'suppress_duplicates_basis', 'sdb', 'cache_basis', 'cb'
            ]:  # See: sr_post.1 sr_watch.1
            ...
            elif words0 in ['chmod', 'default_mode',
                            'dm']:  # See: sr_config.7.rst
                self.chmod = int(words[1], 8)
                n = 2

            elif words0 in ['chmod_dir', 'default_dir_mode',
                            'ddm']:  # See: sr_config.7.rst
            ...
            elif words0 in ['cluster', 'cl', 'from_cluster',
                            'fc']:  # See: sr_config.7
                self.cluster = words1
                ...
                for conf in current_dir_confs:
                    ok, include = self.config_path(
                        self.config_dir, conf, mandatory=True, ctype='inc')
                    self.config(include)
                if not current_dir_confs:
                    config_dir_confs = glob.glob(
                        self.user_config_dir + os.sep + self.program_dir +
                        os.sep + words1)
                ...
                if not ok:
                    self.logger.error(
                        "could not understand destination (%s)" % urlstr)
                    needexit = True
                n = 2
            ...
            elif words0 in ['discard', 'd',
                            'download-and-discard']:  # sr_subscribe.1
                if (words1 is None) or words[0][0:1] == '-':
                    self.discard = True
                    n = 1
                else:
            ...
            elif words0 in ['document_root',
                            'dr']:  # See sr_post.1,sarra,sender,watch
                path = os.path.abspath(words1)
                if self.realpath_post:
                    path = os.path.realpath(path)
                ...
                if 'deleted' in words[1]:
                    self.logger.warning(
                        "deprecated Event spec: please change 'deleted' --> 'delete'"
                    )
                    words[1] = words[1].replace("deleted", "delete")

                if 'created' in words[1]:
                    self.logger.warning(
                        "deprecated Event spec: please change 'created' --> 'create'"
                    )
                    words[1] = words[1].replace("created", "create")

                if 'linked' in words[1]:
                    self.logger.warning(
                        "deprecated Event spec: please change 'linked' --> 'link'"
                    )
                    words[1] = words[1].replace("linked", "link")

                if 'modified' in words[1]:
                    self.logger.warning(
                        "deprecated event spec: please change 'modified' --> 'modify'"
                    )
            ...
            elif words0 in ['exchange',
                            'ex']:  # See: sr_config.7 ++ everywhere fixme?
                self.exchange = words1
                n = 2

            elif words0 in ['exchange_suffix'
                            ]:  # FIXME: sr_config.7 ++ everywhere fixme?
                self.exchange_suffix = words1
                n = 2

            elif words0 in ['expire',
                            'expiry']:  # See: sr_config.7 ++ everywhere fixme?
                if words1.lower() == 'none':
                    ...
                    if self.expire < 300000:
                        self.logger.warning(
                            "expire setting (%s) may cause problem...too low" %
                            words[1])
                n = 2
            ...
            elif words0 in ['follow_symlinks',
                            'fs']:  # See: sr_post.1, sr_watch.1
                if (words1 is None) or words[0][0:1] == '-':
            ...
            elif words0 in ['force_polling',
                            'fp']:  # See: sr_post.1, sr_watch.1
                if (words1 is None) or words[0][0:1] == '-':
                ...
                if key.lower() == "sum":
                    glob_lst = []
                    glob_lst.extend(
                        glob.glob(word) for word in words[2:] if word != [])
                    ...
                    for xfile in file_lst:
                        try:
                            ...
                            x.set('mtime', xmtime)
                            self.logger.debug(
                                "xattr sum set for file: {0} => {1}".format(
                                    xfile, value))
                            x.persist()
                        except:
                            self.logger.error(
                                "could not setxattr (permission denied?)")
                            self.logger.debug(
                                'Exception details: ', exc_info=True)

                n = 2

            elif words0 in ['gateway_for',
                            'gf']:  # See: sr_config.7, sr_sarra.8, sr_sender.1
                ...
                os._exit(0)
                            ...
                            ]:  # See: dd_subscribe (obsolete option...ok)
            ...
            elif words0 in ['inline_encoding', 'inlenc',
                            'content_encoding']:  # See: sr_config.7

                encoding_choices = ['guess', 'text', 'binary']
                if words1.lower() in encoding_choices:
                    self.inline_encoding = words1.lower()
                else:
                    self.logger.error("inline_encoding must be one of: %s" %
                                      encoding_choices)

                n = 2

            elif words0 in ['inline_max', 'imx',
                            'content_max']:  # See: sr_config.7
                self.inline_max = int(words[1])
                n = 2

            elif words0 in ['inplace', 'in', 'assemble'
                            ]:  # See: sr_sarra.8, sr_post.1, sr_watch.1
                if (words1 is None) or words[0][0:1] == '-':
                    self.inplace = True
                    n = 1
                else:
                ...
                self.logger.warning(
                    "deprecated *interface* option no longer has any effect, vip is enough."
                )
                ...
                n = 2
                            ...
                            'inflight']:  # See: sr_config.7, sr_subscribe.1
                if words0 in ['lock']:  # FIXME: remove support in 2019.
                    self.logger.warning(
                        "Deprecated option. Please use *inflight* instead of *lock*"
                    )
                ...
                if (words1 is None) or words[0][0:1] == '-':
                    ...
                    'restore_to_queue', 'restore2queue', 'r2q', 'rq',
                    'post_queue'
            ]:
            ...
            elif words0 in ['report_exchange', 'lx',
                            'le']:  # See: sr_config.7 ++ everywhere fixme?
                self.report_exchange = words1
                n = 2

            elif words0 in ['logdays', 'ld', 'logrotate',
                            'lr']:  # See: sr_subscribe.1
            ...
            elif words0 in ['manager',
                            'feeder']:  # See: sr_config.7, sr_sarra.8
                urlstr = words1
                ok, url = self.validate_urlstr(urlstr)
                self.manager = url
                ...
                if words1.lower() == 'none':
                    ...
                    self.message_ttl = int(
                        self.duration_from_str(words1, 'ms'))
                    if self.message_ttl < 300000:
                        self.logger.warning(
                            "message_ttl setting (%s) may cause problem...too low"
                            % words[1])
                n = 2
            ...
            elif words0 in ['notify_only', 'n',
                            'no_download']:  # See: sr_subscribe.1
            ...
            elif words0 in ['on_heartbeat', 'on_hb'
                            ]:  # See: sr_config.7, sr_sarra,shovel,subscribe
            ...
            elif words0 in ['on_message',
                            'on_msg']:  # See: sr_config.1, others...
            ...
            elif words0 in ['on_report',
                            'on_rpt']:  # FIXME ... sr_config.1, others...
            ...
            # adding paths in command line might be a mess...
            ...
            elif words0 in ['post_base_dir',
                            'pbd']:  # See: sr_sarra,sender,shovel,winnow
            ...
            elif words0 in ['post_broker',
                            'pb']:  # See: sr_sarra,sender,shovel,winnow
                urlstr = words1
                ok, url = self.validate_urlstr(urlstr)
            ...
            elif words0 in ['post_document_root',
                            'pdr']:  # See: sr_sarra,sender,shovel,winnow
                ...
                n = 2
                            ...
                            'px']:  # See: sr_sarra,sender,shovel,winnow
                self.post_exchange = words1
                n = 2

            elif words0 in ['post_exchange_suffix'
                            ]:  # FIXME: sr_sarra,sender,shovel,winnow
            ...
            elif words0 in ['post_topic_prefix',
                            'ptp']:  # FIXME: sr_sarra,sender,shovel,winnow
            ...
            elif words0 in ['post_exchange_split', 'pes',
                            'pxs']:  # sr_config.7, sr_shovel.1
                ...
                if (words1 is None) or words[0][0:1] == '-':
                    ...
                    'queue', 'queue_name', 'qn'
            ]:  # See:  sr_config.7, sender, shovel, sub, winnow too much?
                ...
                n = 2
                    ...
                    'queue_suffix'
            ]:  # See: sr_consumer.1 : but not very usefull... could be removed
            ...
            elif words0 in ['realpath_post', 'realpath',
                            'real']:  # See: sr_post.1, sr_watch.1
                if (words1 is None) or words[0][0:1] == '-':
            ...
            elif words0 in ['rename',
                            'rn']:  # See: sr_poll, sarra, sender, sub, watch?
                self.rename = words1
                n = 2

            elif words0 in ['report_back', 'reportback',
                            'rb']:  # See: sr_subscribe.1
                if (words1 is None) or words[0][0:1] == '-':
                    self.reportback = True
                    n = 1
                else:
                ...
                if words0 in ['role']:
                    self.logger.warning(
                        "role option deprecated, please replace with 'declare'"
                    )
                ...
                if item in ['source', 'subscriber', 'subscribe']:
                    ...
                    self.logger.error("declare not understood: %s %s" %
                                      (item, words2))
                ...
                if not ok:
                    self.logger.error(
                        "unknown checksum specified: %s, should be one of: %s or z"
            ...
            elif words0 in ['post_base_url', 'pbu', 'url', 'u',
                            'post_url']:  # See: sr_config.7
                if words0 in ['url', 'u']:
                    self.logger.warning(
                        "option url deprecated please use post_base_url")
                ...
                if ((words1 is None) or words[0][0:1] == '-'
                    ) and not self.use_pika and amqplib_available:
                ...
                if ((words1 is None) or words[0][0:1] == '-'
                    ) and not self.use_amqplib and pika_available:
                    self.use_pika = True
                    n = 1
            ...
            elif words0 in ['xattr_disable', 'xattr_disabled',
                            'xd']:  # FIXME! what is this?
                if (words1 is None) or words[0][0:1] == '-':
                    xattr_disabled = True
                    n = 1
                else:
                ...
                if not hasattr(self, words[0]):
                    self.logger.debug("unrecognized option %s %s" % (words[0],
                                                                     value))
                    setattr(self, words[0], [value])
                    self.extended_options.append(words[0])
                    self.logger.debug("extend set %s = '%s'" %
                                      (words[0], getattr(self, words[0])))
                else:
                    value2 = getattr(self, words[0])
                    value2.append(value)
                    setattr(self, words[0], value2)
                    self.logger.debug("extend add %s = '%s'" %
                                      (words[0], getattr(self, words[0])))

        except:
            self.logger.error(
                "sr_config/option 4: problem evaluating option %s" % words[0])
            self.logger.debug('Exception details: ', exc_info=True)
    ...
    def setlog(self, interactive=False):
        ...
        if interactive or not self.logpath:
            ...
            self.logger.debug("logging to the console with {}".format(
                self.logger))
        else:
            ...
            self.logger.debug("logging to file ({}) with {}".format(
                self.logpath, self.logger))
    ...
    def validate_parts(self):
        ...
        if not self.parts[0] in ['0', '1', 'p', 'i']:
            self.logger.error(
                "parts invalid strategy (only 0,1,p, or i)(%s)" % self.parts)
            return False
        ...
        if len(token) > 1:
            if self.partflg == '1':
                self.logger.error(
                    "parts invalid strategy 1 (whole files) accepts no other options: (%s)"
                    % self.parts)
                return False
            if self.partflg == 'p':
                self.logger.error(
                    "parts invalid strategy p arguments partial file posting not supported (%s)"
                    % self.parts)
                return False
            ...
            if (self.partflg == 'i' or self.partflg == '0'):
                if len(token) > 2:
                    self.logger.error(
                        "parts invalid too much  (%s)" % self.parts)
                    return False
                ...
                try:
                    ...
                    self.logger.error(
                        "parts invalid blocksize given (%s)" % self.parts)
                    return False
    ...
    def validate_sum(self):
        ...
        if not sumflg in ['L', 'R'] and not sumflg in self.sumalgos:
            return False
        ...
        try:
            ...
            self.logger.error(
                "sr_config/validate_sum 5: sum invalid (%s)" % self.sumflg)
            self.logger.debug('Exception details: ', exc_info=True)
            return False
        return True
    ...
    def wget_config(self, urlstr, path, remote_config_url=False):
        ...
        try:
            ...
            if os.path.isfile(path):
                try:
                    ...
                    if last_mod_remote <= last_mod_local:
                        self.logger.info("file %s is up to date (%s)" %
                                         (path, urlstr))
                        return True
                except:
                    self.logger.error(
            ...
            if os.path.isfile(path):
                self.logger.warning('file %s could not be processed1 (%s)' %
                                    (path, urlstr))
            ...
            if os.path.isfile(path):
                self.logger.warning('file %s could not be processed2 (%s)' %
                                    (path, urlstr))
                self.logger.warning('resume with the one on the server')
            else:
                self.logger.error('Download failed: %s' % urlstr)
                self.logger.error(
                    'Failed to reach server. Reason: %s' % e.reason)

        except:
            if os.path.isfile(path):
                self.logger.warning('file %s could not be processed3 (%s)' %
                                    (path, urlstr))
                self.logger.warning('resume with the one on the server')
            else:
                self.logger.error('Download failed: %s' % urlstr)
    ...
    def set_dir_pattern(self, cdir):
        ...
        if '${DR}' in cdir and self.document_root != None:
            self.logger.warning(
                "DR = document_root should be replaced by BD for base_dir")
            new_dir = new_dir.replace('${DR}', self.document_root)

        if '${PDR}' in cdir and self.post_base_dir != None:
            self.logger.warning(
                "PDR = post_document_root should be replaced by PBD for post_base_dir"
            )
