...
class BaseURLDownloader(object):
    ...
    def do_download(self, parent):
        ...
        wdata = client.service.search(
            args[0], float(args[1]), float(args[2]), float(args[3]),
            float(args[4]), float(args[5]), float(args[6]), args[7], args[8],
            int(args[9]), int(args[10]), args[11], args[12], args[13])
