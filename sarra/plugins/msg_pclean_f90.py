...
class Msg_Pclean_F90(object):
    ...
    def log_compare(self, parent):
        ...
        # compare all files against the first one downloaded
        if not filecmp.cmp(original, self.subs_f30_path):
            logger.error("file %s and %s differs" % (original, self.subs_f30_path))
        if not filecmp.cmp(original, self.send_f50_path):
            logger.error("file %s and %s differs" % (original, self.send_f50_path))
        if not filecmp.cmp(original, self.subs_f60_path):
            logger.error("file %s and %s differs" % (original, self.subs_f60_path))
        if not filecmp.cmp(original, self.subs_f70_path):
            logger.error("file %s and %s differs" % (original, self.subs_f70_path))
        if not filecmp.cmp(original, self.subs_f71_path):
            logger.error("file %s and %s differs" % (original, self.subs_f71_path))
        if not filecmp.cmp(original, self.flow_post_cp):
            logger.error("file %s and %s differs" % (original, self.flow_post_cp))

    def log_state(self, parent, propagated):
        logger = parent.logger

        if not os.path.exists(self.sarr_f20_path):
            logger.debug("file (%s) not found" % self.sarr_f20_path)
        if not os.path.exists(self.subs_f30_path):
            logger.debug("file (%s) not found" % self.subs_f30_path)
        if not os.path.exists(self.send_f50_path):
            logger.debug("file (%s) not found" % self.send_f50_path)
        if not os.path.exists(self.subs_f60_path):
            logger.debug("file (%s) not found" % self.subs_f60_path)
        if not os.path.exists(self.subs_f70_path):
            logger.debug("file (%s) not found" % self.subs_f70_path)
        if not os.path.exists(self.subs_f71_path):
            logger.debug("file (%s) not found" % self.subs_f71_path)
        if not os.path.exists(self.flow_post_cp):
            logger.debug("file (%s) not found" % self.flow_post_cp)
