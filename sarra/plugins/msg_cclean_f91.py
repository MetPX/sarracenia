...
class Msg_Cclean_F91(object):
    ...
    def log_state(self, parent, propagated, ext=None):
        ...
        if not ext:
            ...
            if not os.path.isfile(self.subs_f21_path + ext):
                logger.warning(
                    "file (%s) not found" % self.subs_f21_path + ext)
            if not os.path.isfile(self.subs_f44_path + ext):
                logger.warning(
                    "file (%s) not found" % self.subs_f44_path + ext)
    ...
    def on_message(self, parent):
        ...
        if 'cclean_f90' in msg.headers:
            ...
            logger.info(
                "msg_log received: %s %s%s topic=%s lag=%g %s" % msg_params)
            logger.error(
        ...
        try:
            ...
            logger.warning(
                "file (%s) already deleted ?" % self.subs_f21_path + ext)
