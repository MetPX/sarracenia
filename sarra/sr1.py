...
def instantiate(dirconf, pgm, confname, action):
    ...
    try:
        ...
        if pgm == 'poll': inst = sr_poll(config, [action])
            ...
            cfg.logger.error(
                "code not configured for process type sr_%s" % pgm)
            sys.exit(1)
...
def invoke(dirconf, pgm, confname, action):
    ...
    try:
        ...
        cfg.logger.debug("INVOKE %s %s %s %s" % (program, '-c', confpath,
                                                 action))
...
def main():
    ...
    # actions supported
    actions_supported = [
        'start', 'stop', 'status', 'sanity', 'restart', 'reload', 'cleanup',
        'declare', 'setup', 'remove'
    ]
    ...
    if len(sys.argv) == 1 or sys.argv[1] not in actions_supported:
        print("USAGE: %s %s (version: %s) " % (sys.argv[0], actstr,
                                               sarra.__version__))
        sys.exit(1)
    ...
    if action == 'list':
        if config:
            if config == 'plugins':
                cfg.print_configdir("packaged plugins",
                                    cfg.package_dir + os.sep + 'plugins')
                cfg.print_configdir("user plugins",
                                    cfg.user_config_dir + os.sep + 'plugins')

            else:
                result = cfg.find_conf_file(config)
                if not result:
                    print(
                        "no file named %s found in all sarra configs" % config)
                    sys.exit(1)
            ...
            for d in sorted(cfg.programs):
                cfg.print_configdir(
                    ...
                    cfg.package_dir + os.sep + 'examples' + os.sep + d)
            cfg.print_configdir("general", cfg.user_config_dir)
            for d in sorted(cfg.programs):
                cfg.print_configdir("user configurations",
                                    cfg.user_config_dir + os.sep + d)
        sys.exit(0)
