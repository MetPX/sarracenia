...
class sr_poll(sr_post):
    def cd(self, path):
        try:
            ...
            self.logger.warning(
                "sr_poll/cd: could not cd to directory %s" % path)
        return False
    ...
    def check(self):
        ...
        if self.post_base_url == None:
            ...
            if self.details.url.password:
                self.post_base_url = self.post_base_url.replace(
    ...
    def load_ls_file(self, path):
        ...
        try:
            ...
            self.logger.error(
                "load_ls_file: Unable to parse files from %s" % path)
    ...
    def poll_directory(self, pdir, lspath):
        ...
        if not self.sleeping:
            ...
            self.logger.debug(
                "poll_directory: after differ, len=%d" % len(filelst))
    ...
    def post_new_urls(self):
        ...
        try:
            ...
            self.logger.error("sr_poll/post_new_url: unable to connect to %s" %
                              self.destination)
            self.logger.debug('Exception details: ', exc_info=True)
        ...
        while True:
            ...
            try:
                ...
                if self.vip:
                    ...
                    if self.sleeping:
                        if not last:
                            self.logger.info("%s is sleeping without vip=%s" %
                                             (self.program_name, self.vip))
                    #  active
                    else:
                        if last:
                            self.logger.info("%s is active on vip=%s" %
                                             (self.program_name, self.vip))
                ...
                if not self.sleeping:
                    self.logger.debug(
                        "poll %s is waking up" % self.config_name)
                ...
                if ratio < 0.1:
                    self.logger.warning(
                        "sr_poll sleep too low (%d) secs is less than 10%% of poll time (%f)"
                        % (self.sleep, poll_time))
