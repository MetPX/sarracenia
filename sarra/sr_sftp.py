...
class sr_sftp(sr_proto):
    def __init__(self, parent):
        ...
        try:
            ...
            self.logger.error(
                "sr_sftp/__init__: unable to load ssh config %s" % ssh_config)
    ...
    def connect(self):
        ...
        try:
            ...
            if self.timeout != None:
                self.logger.debug(
                    "sr_sftp connect setting timeout %f" % self.timeout)
            ...
            self.logger.error(
                "sr_sftp/connect: unable to connect to %s (user:%s)" %
                (self.host, self.user))
            self.logger.debug('Exception details: ', exc_info=True)
    ...
    def credentials(self):
        ...
        try:
            ...
            self.logger.debug("h u:p s = %s:%d %s:%s %s" %
    ...
    def get(self,
            ...
            local_offset=0,
            length=0):
        self.logger.debug("sr_sftp get %s %s %d %d %d" %
                          (remote_file, local_file, remote_offset,
                           local_offset, length))
    ...
    def line_callback(self, iline):
        ...
        if not self.parent.ls_file_index in [-1, len(opart2) - 1]:
            fil = ' '.join(opart2[self.parent.ls_file_index:])
        line = ' '.join(opart2)
    ...
    def put(self,
            ...
            remote_offset=0,
            length=0):
        self.logger.debug("sr_sftp put %s %s %d %d %d" %
                          (local_file, remote_file, local_offset,
    ...
    def write_file_index(self):
        ...
        try:
            ...
            self.logger.warning("Unable to write file_index to cache file %s" %
                                self.file_index_cache)
