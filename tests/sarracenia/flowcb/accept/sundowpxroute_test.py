...
def test___init__(caplog, tmp_path):
    ...
    sundewpxroute = SundewPxRoute(options)
    assert sundewpxroute.ahls_to_route == {
        'AACN11_CWLW': True,
        'ABCD11_CYQX': True,
        'INAX13_EUMS': True,
        'WXYZ07_RUHB': True}
...
def test_after_accept(caplog, tmp_path):
    ...
    worklist.incoming = [
        ...
        make_message('/Some/Random/Path/That/Doesnt/Matter/ABCD07_RUHB'),
        # right length, and has _, but not found in ahl keys
        make_message('/Some/Random/Path/That/Doesnt/Matter/WXYZ07_RUHB'),
        # right length, and has _, found in ahl keys
    ]
