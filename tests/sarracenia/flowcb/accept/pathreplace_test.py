...
def test_after_accept():
    options = sarracenia.config.default_config()
    options.pathReplace = [
        'fileReplace,file_foo_Replace',
        'dirReplace,dir_foo_Replace',
        'fileOp_rename,fileOp_foo_rename',
        'fileOp_hlink,fileOp_foo_hlink',
        'fileOp_link,fileOp_foo_link']
    # options.pathReplaceFields = set(['dir','file'])
    options.logLevel = "DEBUG"
    pathreplace = Pathreplace(options)
    ...
    assert worklist.incoming[0]['new_relPath'] == '/Some/dir_foo_Replace/Path' + \
        os.sep + 's0000684_file_foo_Replace_f.xml'
