...
def test_variableExpansion():
    ...
    options,
    message,
    "${PBD}/${SOURCE}/${%Y%m%d}/boughsOfHolly",
    options.post_baseDir +
    '/' +
    message['source'] +
     '/[0-9]{8}/boughsOfHolly')
    assert try_pattern(options, message, "${PBD}/${BUPL}/${%Y%m%d}/falala",
                       options.post_baseDir + '/smorgasbord/[0-9]{8}/falala')
    ...
    options,
    message,
    "a_path/${DONT_TOUCH_THIS}something/else",
     r"a_path/\${DONT_TOUCH_THIS}something/else")
    assert try_pattern(
    options,
    message,
    "a_path/${1DONT_TOUCH_THIS}something/else",
     r"a_path/\${1DONT_TOUCH_THIS}something/else")
    ...
    logger.info(
        "Next is an error message generated because the fourth group does not exist. Expected result is that the ${4} is not replaced.")
...
assert (isinstance(options.queueName, assert () assert (options.queueName.startswith('q_${BROKER_USER}.${COMPONENT}'))
...
def test_multi():
    ...
    options.parse_line("subscribe", "multi1", "subscribe/multi1", 1,
                       "queueName q_${BROKER_USER}.${COMPONENT}.${CONFIG}")
