...
def build_container(blobClient, containerName):
    ...
    # yield azurite_container, dict((key, val) for key, val in [s.split("=",
    # 1) for s in connection_string[:-1].split(";")])
...
def test___credentials():
    ...
    assert transfer.credentials is None

    # Complex, with all options/details
    transfer = sarracenia.transfer.azure.Azure('azure', sarracenia.config.default_config())
    transfer.o.credentials._parse(
        'azure://testing_complex_account.blob.core.windows.net/container azure_storage_credentials=testing_credentials')
    ...
    transfer = sarracenia.transfer.azure.Azure('azure', sarracenia.config.default_config())
    transfer.o.credentials._parse(
        'azblob://testing_complex_account.blob.core.windows.net/container azure_storage_credentials=testing_credentials')
    ...
    transfer = sarracenia.transfer.azure.Azure('azure', sarracenia.config.default_config())
    transfer.o.credentials._parse(
        'azure://test_acct_name:testing%20password@testing_complex_account2.blob.core.windows.net/container')
...
def test_close():
    ...
    transfer.connected = True
    transfer.client = ContainerClient.from_container_url(
        f'https://{TEST_ACCOUNT_NAME}.not.a.real.url.com/{TEST_CONTAINER_NAME}')
...
def test_connect(mocker):
    ...
    with AzuriteContainer(account_name=TEST_ACCOUNT_NAME, account_key=TEST_ACCOUNT_KEY) as azurite_container:
        transfer.container_url = f"http://localhost:{
            azurite_container.get_exposed_port(
                azurite_container.blob_service_port)}/{TEST_ACCOUNT_NAME}/{TEST_CONTAINER_NAME}"
        transfer.container = TEST_CONTAINER_NAME
        transfer.o.sendTo = f'azure://localhost:{
            azurite_container.get_exposed_port(
                azurite_container.blob_service_port)}/{TEST_ACCOUNT_NAME}/{TEST_CONTAINER_NAME}'
...
def test_get(build_client, tmp_path):
    ...
    assert size == len(TEST_CONTAINER_FILES['Folder2/AlsoNestedFile.dat']
...
def test_gethttpsUrl():
    ...
    assert transfer.gethttpsUrl(
        "folder/nestedFolder/File.txt") == f'https://{TEST_ACCOUNT_NAME}.not.a.real.url.com/{TEST_CONTAINER_NAME}/folder/nestedFolder/File.txt'
