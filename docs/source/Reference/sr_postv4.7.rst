...
Version 4 is a proposal for a next iteration of Sarracenia messages, but it is not 
fully implemented, and not in use anywhere, and may never be used. It is also 
not frozen yet and subject to change. version 4 is a GeoJSON representation
adapted from `v3 <sr_post.7.rst>` to be more idiomatic GeoJSON.
...
This format will track work done in the `WMO mesh <https://www.github.com/wmo-im/GTStoWIS2>`_

To generate messages in v03 format, use following setting::

  post_topicPrefix v04
...
Version 04 format of file change announcements for sr_post.  
...
::
           ...
           <version> = "v04" the version of the protocol or format.
...
The headers are an array of name:value pairs::
  ...
  MANDATORY:
          ...
          { "id" : ... relative path from baseUrl (if provided) or logical id.
            "type": "Feature",
            "geometry": ... #as per RFC7946,
            "properties": ... as per RFC7946
                "pubTime"       - YYYYMMDDTHHMMSS.ss - UTC date/timestamp.

  OPTIONAL linke:
          "links":   ... as per RFC7946

  OPTIONAL Properties:

          "baseUrl"       - root of the url to download, when no "links" are provided.
