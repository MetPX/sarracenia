...
téléchargent des données, ils peuvent écrire un message de rapport (report) sur le serveur. Les serveurs sont configurés
pour transférer ces rapports du client via les serveurs intermédiaires vers
...
4. Le rapport est une donnée.
   ...
   *Il ne suffit pas que justice soit faite.  Justice doit être perçue comme étant faite.*
   ...
   et ce rapport doit être renvoyé à la source.  Alors que nous voulons fournir
   ...
   les messages de rapport traversant le réseau jusqu’à la source sont des "dispositions finales"
...
8. Une gestion commune n’est pas nécessaire, il suffit de passer des journaux.
   ...
   les messages de rapport sont routés vers les sources de données, de sorte qu’elles obtiennent nos journaux sur leur
