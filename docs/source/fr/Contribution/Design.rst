...
les messages de rapport signalant l’état des transferts des mêmes données sont acheminés entre
...
Il existe des cas d’utilisation de sécurité très différents pour le transfert de fichiers :
 ...
 1. **Diffusion publique** des données sont produites, dont la confidentialité n’est pas un problème,
    ...
    que les rapports fonctionnent, ce qui permettrait au fournisseur de surveiller et
...
*source* et *consommateur*. Les cas dépendent du routage des messages de notification et des messages de rapport.
...
Les rapport (*reports*) sont définis dans la page de manuel sr_post(7). Ils sont émis par les *consommateurs* à la fin,
ainsi que des *feeders* lorsque les messages traversent les pompes. Les messages de rapport sont publiés sur
l’échange xr_<user> et après validation du rapport mis en fil d’attente pour l’échange xreport.

Les messages en xreport destinés à d’autres clusters sont acheminés vers des destinations par
...
messages de notification, le second est la destination pour envoyer les messages de rapport pour publication
...
Chaque utilisateur Alice, sur un courtier auquel elle a accès :
 ...
 - a un xl_Alice d’échange, où elle écrit ses messages de rapport.
...
Lorsqu’un client comme sarra ou subscribe termine une opération, il crée un message de rapport
...
lors de la création de messages de rapport.

 - Les messages dans les échanges n’ont aucun moyen fiable de déterminer qui les a insérés.
 - Ainsi, les utilisateurs publient leurs messages de rapport sur l'échange sl_<user>.
 - Pour chaque utilisateur, le lecteur de rapport lit le message et écrase l’utilisateur consommateur
 ...
 - Accepter un message de rapport signifie publier sur l’échange xlog.
 ...
 - Rejeter un message de rapport signifie de le copier nulle part.
 ...
 - Le contrôle des ressources n’a pas de sens lorsque des canaux sont utilisés pour le routage inter-pompes.
   ...
   Les pompes recevant les messages de rapport ne doivent pas convertir l’utilisateur consommateur sur ces liens.
   Preuve de la nécessité d’une sorte de réglage : réglage entre l’utilisateur et entre les pompes.


.. NOTE::
   FIXME : si vous rejetez un message de rapport, génère-t-il un message de rapport ?
   Potentiel de déni de service en générant simplement des messages de rapport de tourbières infinis.
