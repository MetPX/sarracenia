...
now = time.time()
print("")
print("%s do product routing from file of %s" % (client, sys.argv[-2]),
      file=sys.stderr)
print("%s file proposed products %d" % (client, NL), file=sys.stderr)
print("%s routing table entries  %d" % (client, len(table_pattern_list)),
      file=sys.stderr)
print("%s accept/reject entries  %d" % (client, len(config_pattern_list)),
      file=sys.stderr)
...
for productline in productfile:
    ...
    print("\r %f%% (%d of %d) spent %f , left %.2d:%.2d:%.2d" %
          (percent, i, NL, span, int(lefthh), int(leftmm), int(leftss)),
          file=sys.stderr,
          end='')
