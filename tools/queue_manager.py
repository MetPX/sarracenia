...
class DeleteQueueNamed(object):
    ...
    def on_channel_open(self, channel):
        ...
        print self.queue
        self._channel.queue_delete(None, self.queue, if_unused=False, if_empty=False, nowait=True)
        self._channel.close()
...
clients = rabbitmqctl("list_consumers | grep -v  ^Listing | grep -v ^...done | awk '{print $1}'")
...
queues = {}
qmm = rabbitmqctl("list_queues name messages memory | grep -v  ^Listing | grep -v ^...done")
...
qx = rabbitmqctl("list_bindings destination_name source_name | grep -v  ^Listing | grep -v ^...done")
